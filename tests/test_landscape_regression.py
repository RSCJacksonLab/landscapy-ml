...
def test_split_indices_accessors_return_copies() -> None:
    ...
    assert splits.counts() == {"train": 2, "val": 1, "test": 1}
...
def test_metrics_and_json_serialization_helpers() -> None:
    ...
    assert regression._correlation(np.array([1.0]), np.array([1.0])) is None
    assert regression._correlation(np.ones(2), np.arange(2)) is None
    assert regression._rankdata(np.array([20.0, 10.0, 10.0])).tolist() == [
        2.0,
        0.5,
        0.5,
    ]
